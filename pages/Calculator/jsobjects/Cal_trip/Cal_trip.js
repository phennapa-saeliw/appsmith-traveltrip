    ...
    if (Select1.selectedOptionValue == '') {
      ...
      await cal_expense.run().then(() => {
        if (cal_expense.data.length != 0) {
          ...
          cal_expense_individualCopy.run().then(() => {
            if (cal_expense_individualCopy.data.length != 0) {
              let expense_individual = cal_expense_individualCopy.data[0].list_expense_amount;
